...
QWidget* SettingsDialog::audio_tab() {
    ...
    //
    // Tight, one group: the checkbox, its device and its debug option
    // are one setting with three controls, not three settings, so they
    // get one trailing note rather than three -- `add_gap` only after
    // it, matching the rest of this dialog's "help belongs to its
    // control" rule.
    diversity_enabled_ =
    ...
    diversity_debug_image_ = new QCheckBox(
        tr("Save a branch-contribution heatmap beside each image"), page);
    diversity_debug_image_->setChecked(config_.receive.diversity_debug_image);
    form->addRow(diversity_debug_image_);
    add_check_note(
        form, page,
        style::note_with_detail(
            tr("The two devices are combined per latent, weighted by each "
               "branch's own confidence."),
            tr("Independent noise and fading, same frequency. The heatmap "
               "(red = this input, blue = the second one) shows which "
               "receiver supplied each part of the image -- diagnostic "
               "only, written as <name>_diversity.png."),
            page));
    add_gap(form);
...
QWidget* SettingsDialog::receive_tab() {
    ...
    add_check_note(form, page,
                   style::note(tr("Low-CPU mode only looks for the start of a "
                                  ...
                                  "in progress or decode retrospectively. Mutually "
                                  "exclusive with Diversity reception, on the "
                                  "Audio tab."),
                               page));
    add_gap(form);
